...
public class BuildJsonForYapi {
    ...
    public static YapiApiDTO actionPerformed(PsiClass selectedClass, PsiMethod psiMethodTarget, Project project, PsiFile psiFile, String attachUpload, String returnClass) {
        // 获取继承的类 BaseController<UserMemberInfoService, UserMemberInfo>
        PsiReferenceList psiReferenceList = selectedClass.getExtendsList();
        String fullClassName = null;
        if (Objects.nonNull(psiReferenceList)) {
            // 获取第一个继承类的第二个泛型对应真正参数 UserMemberInfo
            try {
                PsiType psiType = psiReferenceList.getReferenceElements()[0].getTypeParameters()[1];
                // 获取类的全全限定名 com.xinyartech.erp.user.model.UserMemberInfo
                fullClassName = psiType.getCanonicalText();
            } catch (Exception e) {
        ...
        try {
            ...
            filePaths.clear();
            // 生成响应参数
            yapiApiDTO.setResponse(getResponse(project, psiMethodTarget.getReturnType(), returnClass, fullClassName));
            ...
            getRequest(project, yapiApiDTO, psiMethodTarget, fullClassName);
            if (!Strings.isNullOrEmpty(attachUpload)) {
                if (filePaths.size() > 0) {
                    changeFilePath(project);
     ...
     * @date: 2019/2/19
     */
    public static void getRequest(Project project, YapiApiDTO yapiApiDTO, PsiMethod psiMethodTarget, String fullClassName) throws JSONException {
        ...
        if (psiParameters.length > 0) {
            ...
            for (PsiParameter psiParameter : psiParameters) {
                ...
                if (psiAnnotation != null) {
                    yapiApiDTO.setRequestBody(getResponse(project, psiParameter.getType(), null, fullClassName));
                } else {
                    ...
                    if (psiAnnotation != null) {
                        ...
                        if (psiNameValuePairs.length > 0) {
                            for (PsiNameValuePair psiNameValuePair : psiNameValuePairs) {
                                if ("name".equals(psiNameValuePair.getName()) || "value".equals(psiNameValuePair.getName())) {
                                    if (yapiHeaderDTO != null) {
                                        yapiHeaderDTO.setName(DesUtil.camelToUnderLine(psiNameValuePair.getValue().getText().replace("\"", ""), null));
                                    } else if (yapiPathVariableDTO != null) {
                                        yapiPathVariableDTO.setName(DesUtil.camelToUnderLine(psiNameValuePair.getValue().getText().replace("\"", ""), null));
                                    } else {
                                        yapiQueryDTO.setName(DesUtil.camelToUnderLine(psiNameValuePair.getValue().getText().replace("\"", ""), null));
                                    }
                                    ...
                                    if (yapiHeaderDTO != null) {
                                        yapiHeaderDTO.setName(DesUtil.camelToUnderLine(psiNameValuePair.getLiteralValue(), null));
                                        // 通过方法注释获得 描述 加上 类型
                                    ...
                                    if (yapiPathVariableDTO != null) {
                                        yapiPathVariableDTO.setName(DesUtil.camelToUnderLine(psiNameValuePair.getLiteralValue(), null));
                                        // 通过方法注释获得 描述 加上 类型
                                        ...
                                        yapiQueryDTO.setName(DesUtil.camelToUnderLine(psiNameValuePair.getLiteralValue(), null));
                                        // 通过方法注释获得 描述 加上 类型
                            ...
                            if (yapiHeaderDTO != null) {
                                yapiHeaderDTO.setName(DesUtil.camelToUnderLine(psiParameter.getName(), null));
                                // 通过方法注释获得 描述 加上 类型
                                yapiHeaderDTO.setDesc(DesUtil.getParamDesc(psiMethodTarget, psiParameter.getName()) + "(" + psiParameter.getType().getPresentableText() + ")");
                            } else if (yapiPathVariableDTO != null) {
                                yapiPathVariableDTO.setName(DesUtil.camelToUnderLine(psiParameter.getName(), null));
                                // 通过方法注释获得 描述 加上 类型
                                yapiPathVariableDTO.setDesc(DesUtil.getParamDesc(psiMethodTarget, psiParameter.getName()) + "(" + psiParameter.getType().getPresentableText() + ")");
                            } else {
                                yapiQueryDTO.setName(DesUtil.camelToUnderLine(psiParameter.getName(), null));
                                // 通过方法注释获得 描述 加上 类型
                                yapiQueryDTO.setDesc(DesUtil.getParamDesc(psiMethodTarget, psiParameter.getName()) + "(" + psiParameter.getType().getPresentableText() + ")");
                            }
                            if (NormalTypes.normalTypes.containsKey(psiParameter.getType().getPresentableText())) {
                        ...
                        if (HttpMethodConstant.GET.equals(yapiApiDTO.getMethod())) {
                            List<Map<String, String>> requestList = getRequestForm(project, psiParameter, psiMethodTarget, fullClassName);
                            ...
                            if (yapiApiDTO.getReq_body_form() != null) {
                                yapiApiDTO.getReq_body_form().addAll(getRequestForm(project, psiParameter, psiMethodTarget, fullClassName));
                            } else {
                                yapiApiDTO.setReq_body_form(getRequestForm(project, psiParameter, psiMethodTarget, fullClassName));
                            }
                        }
     ...
     * @date: 2019/5/17
     */
    public static List<Map<String, String>> getRequestForm(Project project, PsiParameter psiParameter, PsiMethod psiMethodTarget, String fullClassName) {
        List<Map<String, String>> requestForm = new ArrayList<>();
        if (NormalTypes.normalTypes.containsKey(psiParameter.getType().getPresentableText())) {
            Map<String, String> map = new HashMap<>();
            map.put("name", DesUtil.camelToUnderLine(psiParameter.getName(), null));
            map.put("type", "text");
            ...
            String canonicalTex = psiParameter.getType().getCanonicalText();
            if ("Model".equals(canonicalTex)) {
                canonicalTex = canonicalTex.replace("Model", fullClassName);
            }
            if ("?".equals(canonicalTex)) {
                canonicalTex = canonicalTex.replace("?", fullClassName);
            }
            PsiClass psiClass = JavaPsiFacade.getInstance(project).findClass(canonicalTex, GlobalSearchScope.allScope(project));
            for (PsiField field : psiClass.getAllFields()) {
                if (field.getModifierList().hasModifierProperty("final")) {
                    continue;
                }
                Map<String, String> map = new HashMap<>();
                map.put("name", DesUtil.camelToUnderLine(field.getName(), null));
                map.put("type", "text");
     ...
     * @date: 2019/2/19
     */
    public static String getResponse(Project project, PsiType psiType, String returnClass, String fullClassName) throws JSONException {
        ...
        if (!Strings.isNullOrEmpty(returnClass) && !psiType.getCanonicalText().split("<")[0].equals(returnClass)) {
            ...
            if (Objects.nonNull(psiClass)) {
                ...
                for (PsiField field : psiClass.getAllFields()) {
                    if (NormalTypes.genericList.contains(field.getType().getPresentableText())) {
                        KV child = getPojoJson(project, psiType, fullClassName);
            ...
            KV kv = getPojoJson(project, psiType, fullClassName);
    ...
    public static KV getPojoJson(Project project, PsiType psiType, String fullClassName) throws JSONException {
        if (psiType instanceof PsiPrimitiveType) {
        ...
        } else if (psiType.getPresentableText().contains("[]")) {
            String[] types = psiType.getCanonicalText().split("\\[");
            KV listKv = new KV();
            if (types.length > 1) {
                String childPackage = types[0];
                if (NormalTypes.noramlTypesPackages.keySet().contains(childPackage)) {
                    String[] childTypes = childPackage.split("\\.");
                    listKv.set("type", childTypes[childTypes.length - 1]);
                }
            ...
            String canonicalTex = psiType.getCanonicalText();
            if (canonicalTex.contains("<Model>")) {
                canonicalTex = canonicalTex.replace("<Model>", "<" + fullClassName + ">");
            }
            if (canonicalTex.contains("<?>")) {
                canonicalTex = canonicalTex.replace("<?>", "<" + fullClassName + ">");
            }
            if ("Model".equals(canonicalTex)) {
                canonicalTex = canonicalTex.replace("Model", fullClassName);
            ...
            String[] types = canonicalTex.split("<");
            if (types.length > 1) {
                ...
                PsiClass psiClassChild = JavaPsiFacade.getInstance(project).findClass(canonicalTex, GlobalSearchScope.allScope(project));
                KV result = new KV();
                List<String> requiredList = new ArrayList<>();
                KV kvObject = getFields(psiClassChild, project, null, null, requiredList, new HashSet<>());
                addFilePaths(filePaths, psiClassChild);
            ...
            if (Objects.nonNull(psiClass.getSuperClass()) && Objects.nonNull(NormalTypes.collectTypes.get(psiClass.getSuperClass().getName()))) {
                for (PsiField field : psiClass.getFields()) {
                    ...
                    if (Objects.nonNull(PsiAnnotationSearchUtil.findAnnotation(field, JavaConstant.NotNull))
                        ...
                        requiredList.add(DesUtil.camelToUnderLine(field.getName(), null));
                    }
    ...
    public static void getField(PsiField field, Project project, KV kv, String[] childType, Integer index, Set<String> pNames) {
        ...
        String name = DesUtil.camelToUnderLine(field.getName(), null);
        String remark = "";
